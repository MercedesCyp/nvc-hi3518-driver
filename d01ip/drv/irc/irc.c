...
#define DRV_NAME                 "irc"
#define DRV_VERSION              "14.11.05 - for D01IPC D04IP"
#define DRV_MINOR                20
...
void img_model(U32 u32_model)
{
	...
	ptask = pid_task(find_vpid(app_pid),PIDTYPE_PID);
	
    //pk("PERI_CRG11 = %lx", ul_data);
	//ul_data = hi_readl(BYPASS);
	//kill_fasync(&irc_devp->async_queue, SIGUSR2, POLL_IN);
...
{	
    ...
    switch(cmd)
    {
		...
		case SET_IRC_USER_PID:
		{
			//maj
			copy_from_user(&app_pid,argp,sizeof(int));
