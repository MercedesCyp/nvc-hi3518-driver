...
#define DC_MQSCSU_ClrRmdSpace 0x04
...
/*  static int sfGetAUserMsg(uint32 iUID,uint8 *piUserMsg,uint16 iMsgLen)
@introduction:
    Accroding the user uuid to read a message from buffer.

@parameter:
    iUID
        User Descriptor
    piUserMsg
        begin of the data buffer start
    iMsgLen
        this  data space size
...
static int sfGetAUserMsg(uint32 iUID, uint8 *piUserMsg, uint16 iMsgLen)
{
	...
	if (_i != DC_MQSpace_RmdMaxUser) {
		...
		mUMsgBody *tNodeTools =
		    (mUMsgBody *) ssManageHeader.aRmdUserInfo[_i].aUMsg;
		if (tNodeTools != NULL) {
			if (tNodeTools->aLen > iMsgLen) {
				return -2;
			} else {
				gClassStr.afCopy(piUserMsg,
				                 ((uint8 *) tNodeTools +
				                  sizeof(mUMsgBody)),
				                 tNodeTools->aLen);
				thRectLen = tNodeTools->aLen;
			}
			ssManageHeader.aRmdUserInfo[_i].aUMsg =
			    tNodeTools->paNext;
	...
	if (_i != DC_MQSpace_SubMaxUser) {
		// get report message
		mUMsgBody *tNodeTools =
		    (mUMsgBody *) ssManageHeader.aSubUserInfo[_i].aUMsg;
		if (tNodeTools != NULL) {
			if (tNodeTools->aLen > iMsgLen) {
				return -2;
			} else {
			...
			ssManageHeader.aSubUserInfo[_i].aUMsg =
			    tNodeTools->paNext;

			if (*ssManageHeader.aSub1stNode == tNodeTools) {
				if (tNodeTools->aUserMask == 0) {
					*ssManageHeader.aSub1stNode =
					    tNodeTools->paNext;
					if (*ssManageHeader.aSub1stNode == NULL) {
						*ssManageHeader.aSubIdleBegin =
						    DC_MQSpace_SubAddrBegin;
						*ssManageHeader.aSubIdleEnd =
						    DC_MQSpace_SubAddrEnd;
						*ssManageHeader.aSubIdleSize =
						    DC_MQSpace_SubDataSize;
					} else {
						*ssManageHeader.aSubIdleEnd =
						    tNodeTools->paNext - 1;
						if (*ssManageHeader.aSubIdleEnd
						    < DC_MQSpace_SubAddrBegin) {
							*ssManageHeader.
							aSubIdleEnd =
							    DC_MQSpace_SubAddrEnd;
...
/*  static int sfMsgQueue_addSubUser(uint32 iUID)
@introduction:
    register the user to recieve the report message
...
/*  static int sfClearMsgBuf(void)
@introduction:
    Delete the user who registered in buffer before
...
static int sfMsgQueue_delSubUser(uint32 iUID)
{
	...
	if (*ssManageHeader.aSubUserNum) {
		...
		if (tStatus & 0x01) {
			...
			*ssManageHeader.aSubIdleEnd =
			    ((void *)*ssManageHeader.aSub1stNode) - 1;
			...
			if (*ssManageHeader.aSubIdleBegin >
			    *ssManageHeader.aSubIdleEnd) {
				*ssManageHeader.aSubIdleSize =
		...
		*ssManageHeader.aSubIdleSize = DC_MQSpace_SubDataSize;
		*ssManageHeader.aSub1stNode = NULL;
		gClassStr.afMemset((uint8 *) ssManageHeader.aSubUserInfo, 0,
...
/*  static int sfIsSomeMsgExist(uint32 iUID)
@introduction:
    Is or not message register in Mqueue
...
/*  static int sfIsUserExist(uint32 iUID)
@introduction:
    Is or not User register in Mqueue
...
/*  static int sfAddAUserMsgForSub(uint8 *piUserMsg, uint16 iMsgLen)
@introduction:
    Add SubMsg To Buffer

@parameter:
    piUserMsg
        The start address of the string use to store into buffer.
    iMsgLen
        String size
...
static int sfAddAUserMsgForSub(uint8 *piUserMsg, uint16 iMsgLen)
{
	...
	if (tSendLen >
	    (1 + DC_MQSpace_SubAddrEnd - (*ssManageHeader.aSubIdleBegin))) {
		*ssManageHeader.aSubIdleBegin = DC_MQSpace_SubAddrBegin;
		*ssManageHeader.aSubIdleSize =
		    1 + *ssManageHeader.aSubIdleEnd -
		    *ssManageHeader.aSubIdleBegin;
...

	{
		...
		tTools_mov = gClassStr.afCopy(tTools_mov, (uint8 *) & tMsgHead,
		                              sizeof(mUMsgBody));
		tTools_mov =
		    gClassStr.afCopy(tTools_mov, (uint8 *) piUserMsg, iMsgLen);
...
static int sfAddAUserMsgForRmd(uint32 iUID, uint8 *piUserMsg, uint16 iMsgLen)
{
	...
	if (*ssManageHeader.aRmdIdleSize < 2 * DC_MQSpace_ManageSize) {
		sfClearStubbornUser(DC_MQSCSU_ClrRmd | DC_MQSCSU_ClrRmdSpace);

	}
	if (tSendLen > (1 + DC_MQSpace_RmdAddrEnd - ((void *)
	                *ssManageHeader.
	                aRmdIdleBegin))) {
		*ssManageHeader.aRmdIdleBegin = DC_MQSpace_RmdAddrBegin;
	...
	if (t32Ret == 0) {
		...
		tStatus |= 0x02;
		t32Ret =
		    sfMQSpace_ManageUser(DC_MQSMU_Cmd_GetRmdUserHNode, iUID,
		                         (void *)&tNode);
	...
	if (tStatus & 0x08) {
		...
		sfMQSpace_ManageUser(DC_MQSMU_Cmd_SetRmdUserHNode, iUID,
		                     &tTools_ori);
	}
...
/*  static uint8 *sfGetMsgBlockAddress(void)
@introduction:
    return the start address of buffer
...
@parameter:
    iCmd
        please notice function definition at document forehead.
    iUUID
        the delete user descriptor
    UMsg
        Read/Write space of user
...
static int32 sfMQSpace_ManageUser(uint32 iCmd, uint32 iUUID, void **UMsg)
{
	...
	if (iCmd & DC_MQSMU_Cmd_SetSubUser) {
		...
		if (*ssManageHeader.aSubUserNum >= DC_MQSpace_SubMaxUser) {
			...
			for (_i = 0; _i < DC_MQSpace_SubMaxUser; _i++)
				if (ssManageHeader.aSubUserInfo[_i].aUID ==
				    iUUID) {
		...
		if (*ssManageHeader.aRmdUserNum >= DC_MQSpace_RmdMaxUser) {
			...
			for (_i = 0; _i < DC_MQSpace_RmdMaxUser; _i++)
				if (ssManageHeader.aRmdUserInfo[_i].aUID ==
				    iUUID) {
					return DC_MQSMU_Err_UserExist;
				}
			for (_i = 0; _i < DC_MQSpace_RmdMaxUser; _i++)
			...
			ssManageHeader.aRmdUserInfo[*ssManageHeader.
			                            aRmdUserNum].aUID = iUUID;
			ssManageHeader.aRmdUserInfo[*ssManageHeader.
			                            aRmdUserNum].aUMsg = NULL;
	...
	} else if (iCmd & DC_MQSMU_Cmd_SetRmdUserHNode ||
	           iCmd & DC_MQSMU_Cmd_GetRmdUserHNode ||
	...
	if (iCmd & DC_MQSMU_Cmd_GetRmdUserHNode) {
...
/*  static int32 sfClearStubbornUser(uint8 iCmd)
@introduction:
    This function shouldnt make misstake, so no matter wath happened, we must
    fix it and keep it return 0 normally and steadily.

@parameter:
    iCmd
        DC_MQSCSU_ClrSub
            clear report user space and message space.

        c
            clear realtime user space

        DC_MQSCSU_ClrRmdSpace
            clear realtime message space, collocation with `DC_MQSCSU_ClrRmdSpace`
            to use
...
static int32 sfClearStubbornUser(uint8 iCmd)
{
	...
	if (iCmd & DC_MQSCSU_ClrSub) {
		...
		if (*ssManageHeader.aSubUserNum) {
			...
			*ssManageHeader.aSubIdleSize = DC_MQSpace_SubDataSize;
			*ssManageHeader.aSub1stNode = NULL;
			gClassStr.afMemset((uint8 *) ssManageHeader.
			                   aSubUserInfo, 0,
			                   sizeof(mMQUserMsg) *
			                   DC_MQSpace_SubMaxUser);
			*ssManageHeader.aSubUserNum = 0;
		}
		...
		for (_i = 0; _i < DC_MQSpace_RmdMaxUser; _i++) {
			uint8 tUserNumber = 0;
			uint8 tUserMaxEntry = 0;
			if (ssManageHeader.aRmdUserInfo[_i].aUID != 0) {
				if (ssManageHeader.aRmdUserInfo[_i].aUMsg ==
				    NULL) {
					ssManageHeader.aRmdUserInfo[_i].aUID =
					    0;
					(*ssManageHeader.aRmdUserNum)--;
				} else {
					uint8 tEntry = 0;
					mUMsgBody *tTool =
					    ssManageHeader.aRmdUserInfo[_i].
					    aUMsg;
					while (tTool != NULL) {
						tEntry++;
						tTool = tTool->paNext;
					}
					if (tEntry > tUserMaxEntry) {
						tUserMaxEntry = tEntry;
						tUserNumber = _i;
					}
				}
			}

			if (iCmd & DC_MQSCSU_ClrRmdSpace) {

				if (tUserNumber != 0) {
					uint8 _j;
					void *t1stNode =
					    DC_MQSpace_RmdAddrEnd +
					    DC_MQSpace_RmdDataSize;
					void *tEndNode = NULL;
					void *tLstNode = NULL;
					...
					ssManageHeader.aRmdUserInfo
					[tUserNumber].aUMsg = 0;
					ssManageHeader.aRmdUserInfo
					[tUserNumber].aUID = 0;
					(*ssManageHeader.aRmdUserNum)--;
					for (_j = 0; _j < DC_MQSpace_RmdMaxUser;
					     _j++) {
						if ((ssManageHeader.aRmdUserInfo
						     [_j].aUID != 0)
						    ||
						    (ssManageHeader.aRmdUserInfo
						     [_j].aUMsg != 0)) {
					...
					if (tEndNode != NULL) {
						...
						*ssManageHeader.aRmdIdleEnd =
						    tEndNode - 1;
						if (*ssManageHeader.aRmdIdleEnd
						    < DC_MQSpace_RmdAddrBegin) {
							*ssManageHeader.
							aRmdIdleEnd =
							    DC_MQSpace_RmdAddrEnd;
						}
					...
					if (*ssManageHeader.aRmdIdleBegin >
					    *ssManageHeader.aRmdIdleEnd) {
						*ssManageHeader.aRmdIdleSize =
						    (void *)
						    (*ssManageHeader.
					...
					for (_j = 0; _j < DC_MQSpace_RmdMaxUser;
					     _j++) {
						ssManageHeader.aRmdUserInfo[_j].
						aUID = 0;
						ssManageHeader.aRmdUserInfo[_j].
						aUMsg = NULL;
					}
...
@introduction:
    Show MQueue Manage information.
...
void DbgShowTheManagerField(void)
{
	...
	DbgPrinStr("Sub-->UserInfo:\r\n",
	           (uint8 *)(ssManageHeader.aSubUserInfo),
	           sizeof(mMQUserMsg) * DC_MQSpace_SubMaxUser);
	...
	DbgPrinStr("Rmd-->UserInfo:\r\n",
	           (uint8 *)(ssManageHeader.aRmdUserInfo),
