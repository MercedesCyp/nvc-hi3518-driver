...
{
    S32 s32_sensor_select_fd;

    s32_sensor_select_fd = open("/dev/sensor_select", 0);

    if(s32_sensor_select_fd<0)
    {
        printf("err: open sensor select!\n");
        return -1;
    }
	
    if(ioctl(s32_sensor_select_fd, SET_SEN_DAY_EN) != 0)
    {
        perror("ioctl-- SET_SEN_DAY_EN error\n");
		return -1;
	}
	...
	if(ioctl(s32_sensor_select_fd, SET_SEN_NIGHT_EN) != 0)
	{
	    perror("ioctl-- SET_SEN_NIGHT_EN error\n");
		return -1;
	}

    sensor_init();

    printf("two sensor init Compile Time = %s %s\n", __DATE__, __TIME__);
    printf("two sensor init success.\n");
	return 0;
}
