...
void com_head(void)
{
    ...
    printf(" 1) get button status                      2) monitor button status\n");
    printf(" 0) quit");
    printf("\n");
...
void button_monitor_thread(void * arg)
{
	...
	while(pthread_run == 1)
	{
		...
		if ((nSize = nv_drv_wait_message(s32NvcFd, g_fDriver_version >= 1.0 ? NVC_REPORT_BUTTON_STATUS_MSG : NVC_OLD_REPORT_BUTTON_STATUS_MSG, szBuffer, sizeof(szBuffer), 2000)) <= 0)
			continue;

		printf("\nButton %d %s\n", pDrvMsg->stMsgHdr.u8DevNo, pDrvMsg->stButtonStatus.u8Status == Nvc_ButtonActionRaise ? "loosen up" : "press down");
	}
}
...
int main()
{
	...
	if ((stCapInfo.u32CapMask & NVC_SUPP_ButtonMonitor) == 0 || stCapInfo.u8ButtonCnt <= 0)
	{
		NV_ERROR("Dont support button for this device.");
		close(s32NvcFd);
		return -3;
	}
	system("clear");
	...
	//	  printf("Had input %d\n",ichose);
		...
		{ 
			case 1:
			{
			   if (stCapInfo.u8ButtonCnt > 1)
			   {
			   		printf("Select button no [%d~%d]:", 0, stCapInfo.u8ButtonCnt - 1);
					scanf("%d", &nButtonNo);
					if (nButtonNo < 0 || nButtonNo >= stCapInfo.u8ButtonCnt)
					{
						printf("Button no error!\n");
						break;
					}

			   }
			   if(iret = nv_drv_get_button_status(s32NvcFd, nButtonNo, &enState))
			   {
				   printf(" iret = %d\n", iret);
				   break;
				}
				...
				{
					if(enState == NV_BUTTON_LOOSEN_UP)
					{
						printf("Button loosen up!\n");
					}
					if(enState == NV_BUTTON_PRESS_DOWN)
					{
			...
			case 2:
			{
				...
				if (0 != pthread_create(&pthread_id, NULL, (void* (*)(void*))button_monitor_thread, NULL))
				{
